...
## Multipage CRUD Application built with Python's Flask Framework Using a MongoDB
...
The planning commenced with pen and paper brainstorming and some competitor analysis. Here ideas for a twist on a regular to-do app were considered. Bullet journaling is a widely used and fast trending practice of using a notebook to organise and track all aspects of a busy life. I viewed many videos along with a Ted talk by the founder of this concept and decided to explore this as a theme for my data centric project. Competitor analysis was then carried out on some to-do list applications for mobile and desktop. Other mobile and desktop application interfaces were also reviewed using resources such as Pinterest and Dribble for guidance on usability. One of the challenges of this development was to create an application with good functionality while also fitting this into a simple interface which is easy to use and responsive for using on mobile devices. 
...
The following is a list of some user stories:
 ...
 - As a user seeking an application to store to do lists, I want to be able to have quick access to view my lists and also add items quickly on the go, this will help me not to forget anything and ensure that everything that I think of is recorded.
...
- The user can create, read, update, and delete all items and categories with ease.
- In addition to storing lists the user can add bullets/icons to denote firstly whether the item is a to-do, event or just simply a note. 
...
- JavaScript & [jQuery](https://jquery.com/Jquery "jQuery Homepage")  - Used for the typewriter effect on the desktop view for the homepage. Materialize also uses these for some of its navigation features e.g. accordions and side navigation for mobile devices.
...
- Manual testing was carried out on this site. Some of the items found include:
  - All form fields were tested to see what what would happen if left blank, in some cases this caused issues. To resolve this, I put in a "required" tag against relevant fields.
  ...
  - All aspects of CRUD functionality were tested to ensure that the correct database records were updated.
...
3. The requirements.txt contains all the dependencies required to run the app.
4. The Procfile communicates to Heroku how to run the app.
5. The server used for hosting the dataset is [mLab](https://mlab.com  "mLab Homepage") MongoDB. You will need to create your own connection and access keys. These are stored in Heroku config vars in the "Settings" tab.
